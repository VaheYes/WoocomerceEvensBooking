...
if (! class_exists ( 'DTPageBuilder' )) {
	...
	class DTPageBuilder {
		...
		function save_post_meta( $post_id, $post ){
				...
				// Gutenberg Compatability Scripts Start
				$post_content = $post->post_content; // Get the post_content
				preg_match_all('<!-- /wp:(.*?) -->', $post_content, $blocks);

				if(isset($blocks[0]) && !empty($blocks[0])) {
					return;
				}
				// Gutenberg Compatability Scripts End
				...
				add_filter( 'redirect_post_location', array( $this, 'dt_update_query_var' ), 99 );
				
		}

		public function dt_update_query_var( $location ) {
			remove_filter( 'redirect_post_location', array( $this, 'dt_update_query_var' ), 99 );
			return add_query_arg( array( 'classic-editor' => 1 ), $location );
        }

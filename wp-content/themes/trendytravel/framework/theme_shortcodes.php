...
if(!function_exists('dt_contact_form')) {
	...
	function dt_contact_form( $atts, $content = null ) {
		...
		$out .= '<form class="wpcf7-form contact-frm" method="post" action="'.get_template_directory_uri().'/framework/sendmail.php">';
			...
			$out .=' <input id="dtcontactfrm" type="submit" name="submit" value="'.__('Send Message', 'iamd_text_domain').'" />
					</p>';
...
if(!function_exists('dt_social')) {
	...
	function dt_social($attrs, $content=null,$shortcodename="") {
		...
		if(!empty($dt_theme_options['social'])):
			$out .= $text;
...
if(!function_exists('googleplusone')) {
...
if(!function_exists('stumbleupon')) {
...
//SINGLE IMAGE...
if(!function_exists('dt_single_image')) {

	function dt_single_image( $atts, $content = null ) {
		extract(shortcode_atts(array(
			'title' => '',
			'url'   => 'http'.dt_theme_ssl().'://placehold.it/300x300',
			'class' => ''
		), $atts));

		$out = '<img src="'.$url.'" title="'.$title.'" alt="'.$title.'" class="'.$class.'" />';

		return $out;
	}
	add_shortcode('dt_single_image', 'dt_single_image');
}

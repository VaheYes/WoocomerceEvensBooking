...
if( !class_exists( 'dt_theme_privacy_class' ) ) {
	...
	class dt_theme_privacy_class {
		...
		function __construct() {
			...
			$privacy_reservationfrm = dt_theme_option('privacy','enable-reservationfrm-msg');
		...
		function dt_theme_move_textarea( $input = array () ) {
			...
			if ( 'comment_form_defaults' === current_filter() ) {
				...
				$input['label_submit'] = esc_html__('Comment', 'iamd_text_domain');
				$input['title_reply'] = esc_html__( 'Give a Reply', 'iamd_text_domain' );
		...
		function dt_theme_privacy_verify_comment_checkbox( $commentdata ) {
		    ...
		    if ( ! is_user_logged_in() && ! isset( $_POST['comment-form-dt-privatepolicy'] ) ) {
			    $error_message = apply_filters( 'dt_theme_privacy_comment_checkbox_error_message', __( 'Error: You must agree to our privacy policy to comment on this site...' , 'iamd_text_domain' ) );
		...
		function dt_theme_privacy_contact_checkbox( $content = "", $attrs ) {
			...
			$output = '<div class="dt-privacy-wrapper">';
				$output .= '<input name="txtprivacy" id="dt_contact_privacy" type="checkbox" required="required"><label for="dt_contact_privacy">'.$content.'</label>';
			$output .= '</div>';
		...
		function dt_theme_privacy_reservation_checkbox( $content = "", $attrs ) {
			...
			if( empty($content) ) 
				...
				$content = do_shortcode( dt_theme_option('privacy', 'reservationfrm-msg') );

			$output = '<div class="dt-privacy-wrapper">';
				$output .= '<input name="bookprivacy" id="dt_reservation_privacy" type="checkbox" required="required"><label for="dt_reservation_privacy">'.$content.'</label>';
			$output .= '</div>';
		...
		function dt_theme_authenticate_user_acc( $user, $password ) {
		    ...
		    if ( isset( $_REQUEST['comment-form-dt-privatepolicy'] ) ) {
		        ...
		        $error = new WP_Error();
		        $error->add('did_not_accept', __( 'You must acknowledge and agree to the privacy policy' , 'iamd_text_domain'));
		        return $error;
		    }
		}
		...
		if( isset( $cookie_bar ) ) {
            ...
            <div class="dt-cookie-consent cookiebar-hidden dt-cookiemessage-<?php echo esc_attr($position); ?>">
	            <div class="container">
					...
					if( empty( $buttons ) ):
						?><a href="#" class="dt-sc-button filled small dt-cookie-consent-button dt-cookie-close-bar" data-contents="<?php echo esc_attr($cookie_contents); ?>"><?php esc_html_e('OK', 'iamd_text_domain'); ?></a><?php
					endif;
					...
					if( !empty( $buttons ) ):
						foreach($buttons as $button) {
							...
							if( !empty( $button['action'] ) && $button['action'] == 'info_modal' ) {
								...
								$contents[] = array(	'label'		=> __( 'Other external services', 'iamd_text_domain' ), 
				...
				[dt_sc_privacy_video_embeds]', 'iamd_text_domain' )
								);

								$wp_privacy_page = get_option('wp_page_for_privacy_policy');
								if( !empty( $wp_privacy_page ) ) {
									$contents[] = array(	'label'		=> __( 'Privacy Policy', 'iamd_text_domain' ), 
															'content'	=> __( 'You can read about our cookies and privacy settings in detail on our Privacy Policy Page. <br><br> [dt_sc_privacy_link]', 'iamd_text_domain' ));
								}
